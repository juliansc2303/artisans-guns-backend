...
{
    /// <summary>
    /// Lives on the FPV ultimate prefab spawned into WeaponHolder.
    /// Receives animation events from the hands animator (same pattern as SmokeGrenadeAbility).
    ///
    ...
    ///   • Parameter   → Trigger          "Shoot"            (triggered by ThrowUltimate())
    /// </summary>
    public class CrimsonUltimateAbility : MonoBehaviour
    {
        [Tooltip("Projectile prefab to spawn (should have CrimsonUltimateProjectile + Rigidbody)")]
        ...
        private void Awake()
        {
            ...
            if (equipSound == null) equipSound = Resources.Load<AudioClip>("Sounds/UltimateEquip");
            if (throwSound == null) throwSound = Resources.Load<AudioClip>("Sounds/UltimateThrow");
        }
        ...
        public void ThrowUltimate()
        {
            if (!IsReady)
            {
                Debug.Log("[CrimsonUltimateAbility] Not ready yet — waiting for OnAbilityReady animation event");
                return;
            }

            if (animator == null)
            {
                Debug.LogWarning("[CrimsonUltimateAbility] No Animator found — cannot throw");
                return;
            }
        ...
        /// instantiates and launches the ultimate projectile.
        /// </summary>
        public void OnThrowGrenade()
        {
            if (projectilePrefab == null)
            {
                Debug.LogWarning("[CrimsonUltimateAbility] projectilePrefab not assigned");
                return;
            }
            ...
            if (spawnPoint == null)
            {
                ...
                Debug.LogWarning("[CrimsonUltimateAbility] abilitySpawner not set — using transform as fallback");
            }
